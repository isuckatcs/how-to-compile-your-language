...
Sema::resolveBinaryOperator(res::Context &ctx,
  ...
  if (op == TokenKind::EqualEqual) {
    ...
    if (!typeError) {
      lhs = qualificationConvIfNeeded(ctx, lhs, rhsTy);
      rhs = qualificationConvIfNeeded(ctx, rhs, lhsTy);
...
res::CallExpr *Sema::resolveCallExpr(res::Context &ctx,
  ...
  for (auto &&argument : arguments) {
    ...
    auto *qcArgument =
        qualificationConvIfNeeded(ctx, resolvedArgument, expectedType);
    auto *refArgument = coerceToRefIfNeeded(ctx, qcArgument, expectedType);
    auto *promotedArgument =
        tryPromoteToTraitObject(ctx, refArgument, expectedType);
    if (!promotedArgument)
      continue;

    promotedArgument->setConstantValue(cee->evaluate(*promotedArgument));
    resCall->addArg(promotedArgument);
  }
...
res::StructInstantiationExpr *Sema::resolveStructInstantiation(
  ...
  for (auto &&initStmt : structInstantiation.fieldInitializers) {
    ...
    auto *promotedInitExpr = tryPromoteToTraitObject(
        ctx, qualificationConvIfNeeded(ctx, resolvedInitExpr, fieldTy),
        fieldTy);
    if (!promotedInitExpr) {
      error = true;
      continue;
    ...
    inits[id] = resolvedFieldInits.emplace_back(
        res::FieldInitStmt::create(ctx, loc, fieldDecl, promotedInitExpr));
  }
...
res::Expr *Sema::resolveMemberExpr(res::Context &ctx,
  ...
  auto *qcBase = qualificationConvIfNeeded(ctx, base, selfType);
  auto *refBase = coerceToRefIfNeeded(ctx, qcBase, selfType);
...
res::Expr *Sema::qualificationConvIfNeeded(res::Context &ctx,
                                           res::Expr *expr,
                                           res::Type *to) {
  // *mut type -> *type
  // &mut type -> &type
  ...
  auto *qc = res::QualificationConv::create(ctx, expr->location, expr);
  if (to->getAs<res::PointerType>())
    qc->setType(res::PointerType::create(ctx, fromInner, false));
  else
    qc->setType(res::RefType::create(ctx, fromInner, false));

  return qc;
}

res::Expr *
Sema::coerceToRefIfNeeded(res::Context &ctx, res::Expr *expr, res::Type *to) {
  ...
  res::Type *referencedType = toRef->getReferencedType();

  // *type -> &type
  auto *fromPtr = from->getAs<res::PointerType>();
  if (fromPtr && !referencedType->getAs<res::PointerType>()) {
    if (!fromPtr->isMutable() && toRef->isMutable())
      return expr;

    auto *decay = res::ImplicitPtrToRefDecay::create(ctx, expr->location, expr);
    decay->setType(res::RefType::create(ctx, fromPtr->getPointeeType(),
                                        toRef->isMutable()));
    return decay;
  }
  ...
  asRef->setType(res::RefType::create(ctx, from, toRef->isMutable()));
  return asRef;
}

res::Expr *Sema::tryPromoteToTraitObject(res::Context &ctx,
                                         res::Expr *expr,
                                         res::Type *to) {
  // *type -> *any
  // &type -> &any
...
res::Assignment *Sema::resolveAssignment(res::Context &ctx,
  ...
  varOrReturn(promotedRhs,
              tryPromoteToTraitObject(
                  ctx, qualificationConvIfNeeded(ctx, rhs, lhsTy), lhsTy));
...
res::ReturnStmt *Sema::resolveReturnStmt(res::Context &ctx,
  ...
  if (returnStmt.expr) {
    ...
    varOrReturn(
        promotedExpr,
        tryPromoteToTraitObject(
            ctx, qualificationConvIfNeeded(ctx, resolvedExpr, retTy), retTy));
...
res::VarDecl *Sema::resolveVarDecl(res::Context &ctx,
  ...
  if (varDecl.initializer) {
    ...
    varOrReturn(promotedInit,
                tryPromoteToTraitObject(
                    ctx, qualificationConvIfNeeded(ctx, init, declTy), declTy));
